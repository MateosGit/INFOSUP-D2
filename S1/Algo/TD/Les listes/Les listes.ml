...
let eratosthene n= if n<= 2 then
    invalid_arg "ertosthene : n must be a natural above 2"
  else
    let rec suppr_mult mult = function
      [] -> []
    | e::l when e mod mult = 0 -> suppr_mult mult l
    | e::l -> e::suppr_mult mult l
    in let rec increment_mult = function
        [] -> []
      | e::l when e*e>n -> e::l
      | e::l -> e:: increment_mult (suppr_mult e l)
       in 2 :: increment_mult (arith_list ((n-1)/2) 3 2);;
